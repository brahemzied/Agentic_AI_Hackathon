...
curl -sS -X POST "http://localhost:8585/api/v1/services/databaseServices" \
...
pip install "openmetadata-ingestion[postgres]==1.9.11"
metadata ingest -c postgres_ingestion.yml

 pip install "openmetadata-ingestion[dbt]==1.9.11"
metadata ingest-dbt
